<script setup lang="ts">
  ...
  const isBookmarked = computed(() => {
    return cultureStore.bookmarkedFestivals.some(
      (festival) => festival.content_id === festivalId.value,
    );
  });
  ...
  const toggleBookmark = () => {
    if (festivalDetail.value) {
      const newBookmark = {
        ...
        homepage:
          festivalImages.value.length > 0 ? festivalImages.value[0] : '/images/default-image.jpg', // ✅ 첫 번째 이미지 저장
        overview: festivalDetail.value.overview || '설명 없음',
        event_start_date:
          eventIntro.value.event_start_date && eventIntro.value.event_start_date !== '날짜 미정'
            ? eventIntro.value.event_start_date
            : '날짜 미정',
        event_end_date:
          eventIntro.value.event_end_date && eventIntro.value.event_end_date !== '날짜 미정'
