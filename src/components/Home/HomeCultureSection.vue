<template>
  <div>
    ...
    <div class="pt-10">
      <ul class="flex  gap-6">
        <li v-for="(item,index) in festivalData" :key="index"><CultureCard src="https://cdn.danimgil.com/news/photo/202502/11337_11697_5757.jpg" @click="router.push(`/culture/${item.content_id}`)" :content="item.address" :link="item.homepage" :title="item.name" :category="getCategoryName(item.category3)" /></li>
      </ul>
    </div>
  </div>
</template>
